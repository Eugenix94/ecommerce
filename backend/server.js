...
app.post('/api/users/delete', async (req, res) => {
  ...
  console.log('POST /api/users/delete', { username });
  if (!username) {
...
app.post('/api/checkout', async (req, res) => {
  ...
  try {
    ...
    const orderRes = await pool.query(
      'INSERT INTO orders (user_id, total, payment_status, delivery_status, address, payment_method, delivery_method) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id, created_at',
      [userId, total, paymentStatus, deliveryStatus, address, paymentMethod, deliveryMethod]
    );
...
app.get('/api/orders/:userId', async (req, res) => {
  ...
  try {
    ...
    for (const order of orders) {
      const itemsRes = await pool.query('SELECT * FROM order_products WHERE order_id = $1', [order.id]);
